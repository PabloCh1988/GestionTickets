...
async function ObtenerTickets() {
  ...
  const filtro = {
    categoriaId:
      categoriaIdBuscar && categoriaIdBuscar !== "0"
        ? parseInt(categoriaIdBuscar)
        : null,
    estado:
      estadoBuscar && estadoBuscar !== "0" ? parseInt(estadoBuscar) : null,
    prioridad:
      prioridadBuscar && prioridadBuscar !== "0"
        ? parseInt(prioridadBuscar)
        : null,
  ...
  tickets.forEach(item => {
    ...
    fila.innerHTML = `
                ...
                <button class='btn btn-inverse-warning  mdi mdi-file-find' title='Historial' onclick='MostrarHistorial(${item.ticketId})'></button>
                <button class='btn btn-inverse-danger  mdi mdi-refresh' title='Estado' onclick='CambiarEstado(${item.ticketId})'></button>
                </td>
            `;
...
function CambiarEstado(id) {
  authFetch(`tickets/cambiarestado/${id}`, {
    method: "PUT",
  })
    .then(response => {
      if (!response.ok) {
      ...
      ObtenerTickets(); // Refresca la tabla
      Swal.fire({
        position: "top-end",
        icon: "success",
    ...
    .catch(error => {
      ...
      alert("No se pudo cambiar el estado del ticket: " + error.message);
    });
}
...
async function comboCategoriasEditar(selectedId) {
  ...
  categorias.forEach(cat => {
    ...
    combo.innerHTML += `<option value="${id}" ${id == selectedId ? "selected" : ""
      }>${desc}</option>`;
  });
}
      ...
      if (Array.isArray(listado) && listado.length > 0) {
        ...
        $.each(listado, function (index, item) {
          $("#historialTickets").append(
            ...
            "</td>" +
            "<td>" +
            (item.campoModificado === "Categoria"
              ? categoriasMap[item.valorAnterior] || item.valorAnterior
              : item.valorAnterior) +
            "</td>" +
            "<td>" +
            (item.campoModificado === "Categoria"
              ? categoriasMap[item.valorNuevo] || item.valorNuevo
              : item.valorNuevo) +
            "</td>" +
            "<td>" +
